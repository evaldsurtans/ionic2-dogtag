...
class LightBluePacket {
  ...
  static fromBuffer(buf) {
    ...
    //let sbuf = Uint16Array.from(buf);
    let val = buf.readUInt8(0);
    let firstPacket = (val & 0x80) == 0x80;
    let packetCount = (((val & 0x60) >> 5));
    let packetsRemaining = (val & 0x1f);
  ...
  constructor(first, packetCount, packetsRemaining, payload) {
    if (payload.length > 19) {
      throw new Error("Payload length cannot be greater than 19");
  ...
  toString() {
    return `LightBluePacket (First: ${this._first}) (count: ${this._packetCount}) (Remaining: ${this._packetsRemaining})`
  }
...
module.exports = {
 // SerialTransportService: SerialTransportService,
  LightBluePacket: LightBluePacket
}
